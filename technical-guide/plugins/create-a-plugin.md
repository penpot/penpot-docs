...
You can check the examples in [Plugin examples](TBD).
...
| React     | `npx create-react-app plugin-name` | 18.3.1    |
...
- `@penpot/plugin-styles`: contains styles to help build the UI for Penpot plugins.
...
We're using Verdaccio for local publishing. Check the documentation [Publishing packages](https://github.com/penpot/penpot-plugins/blob/main/docs/publish-package.md) for more.
...
```bash
npm i @penpot/plugin-styles @penpot/plugin-types --registry http://localhost:4873
```
...
```bash
npm i @penpot/plugin-styles @penpot/plugin-types
```
...
We used this solution in the Angular example.
...
We used this solution in the Vue example, and also in the vanilla template.
