...
format: `<enable|disable>-<flag-name>`. For example:
...
By default, `smpt` flag is disabled, the email will be printed to the console,
...
If you have an SMTP service, uncomment the appropriate
settings section in `docker-compose.yml` and configure those
...
You will need to set the `PENPOT_PUBLIC_URI` environment variable
in case you go to serve Penpot to the users; it should point to public URI where users
...
- `disable-secure-session-cookies`: By default, Penpot uses the `secure` flag on cookies,
  this flag disables it; it is useful if you plan to serve Penpot under different
